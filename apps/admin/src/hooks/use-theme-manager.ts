...
export function useThemeManager() {
  ...
  const applyTheme = React.useCallback((themeValue: string, darkMode: boolean) => {
    ...
    // Reset and apply theme variables with smooth transition
    resetTheme()
    const styles = darkMode ? theme.preset.styles.dark : theme.preset.styles.light
    const root = document.documentElement

    // Apply theme variables immediately (synchronously) to ensure they're set
    Object.entries(styles).forEach(([key, value]) => {
      root.style.setProperty(`--${key}`, value)
    })
    
    // Also apply in requestAnimationFrame for smooth transitions
  ...
  const applyTweakcnTheme = React.useCallback((themePreset: ThemePreset, darkMode: boolean) => {
    ...
    // Apply theme variables immediately (synchronously) to ensure they're set
    // Then use requestAnimationFrame for smooth transitions if needed
    Object.entries(styles).forEach(([key, value]) => {
      root.style.setProperty(`--${key}`, value)
    })
  ...
  const applyImportedTheme = React.useCallback((themeData: ImportedTheme, darkMode: boolean) => {
    ...
    // Apply all variables from the theme immediately (synchronously) to ensure they're set
  ...
  const handleColorChange = (cssVar: string, value: string) => {
    ...
    // Update brand colors state so it gets saved to database
