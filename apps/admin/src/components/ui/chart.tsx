...
function ChartContainer({
  ...
  config: ChartConfig
  /** When true, always render the chart (skip dimension check). Use when the chart area stays blank. */
  forceMount?: boolean
  ...
  const [isReady, setIsReady] = React.useState(!!forceMount)
  ...
  React.useLayoutEffect(() => {
    ...
    const checkDimensions = () => {
      ...
      // Require both width and height to be greater than 0
      // Also check computed styles to ensure container has dimensions
      const computedStyle = window.getComputedStyle(containerRef.current)
      const hasWidth = rect.width > 0 || computedStyle.width !== 'auto' && computedStyle.width !== '0px'
      const hasHeight = rect.height > 0 || computedStyle.height !== 'auto' && computedStyle.height !== '0px'
      
      if (hasWidth && hasHeight && rect.width > 0 && rect.height > 0) {
        setIsReady(true)
      } else {
        setIsReady(false)
      }
  ...
  const showChart = forceMount || isReady
